...
class MatrizRigidez():
    ...
    def rigidez(self, n_elem):
        ...
        # Definindo a matriz de rigidez elementar de acordo com as condições de contorno
...
# Calculando as matrizes de rigidez elementares
...
# Calculando os deslocamentos para cada elemento
for i in range(1, len(el.Le) + 1, 1):
...
print("\nMatrizes de Rotação:")
for i in range(1, len(el.Le) + 1):
    print(f"\nElemento {i}:")
    print(el.Re[i])



print("\nVetores de Engastamento Perfeito:")
for i in range(1, len(el.Le) + 1):
    if i in cd().forca_eq_l.keys():
        print(f"\nElemento {i}:")
        print(cd().forca_eq_l[i])
